...
def project_client_projection(raw_payload: dict[str, Any]) -> dict[str, Any]:
    """
    Client projection: total, turnaround, safe line items (Print & finishing, Platform fee, Delivery).
...
def project_broker_projection(raw_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    broker_markup slot, client_price, margin. No raw rate card values.
    """
    totals = raw_payload.get("totals") or {}
    ...
    return {
        ...
        "broker_markup": None, # Placeholder for broker to fill
        "client_price": None, # Placeholder
        "margin": None, # Placeholder
    }
...
def _extract_client_line_items(raw_payload: dict[str, Any]) -> list[dict[str, Any]]:
    totals = raw_payload.get("totals") or {}
    # Safe line items: Print & finishing, Platform fee, Delivery
    # Note: Platform fee and Delivery are usually added at the builder or serializer level,
    ...
    # We don't have platform fee in raw engine payload usually (it's internal to engine as subtotal components)
    # But if we are projecting a calculated result that has them:
    if "platform_fee" in totals:
         items.append({"label": "Platform fee", "amount": _money(totals.get("platform_fee"))})
