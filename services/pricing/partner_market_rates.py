...
SAMPLE_QUANTITY = 100
DEFAULT_MARKUP_RATE = Decimal("0.30")
...
def _active_shop_rows() -> dict[str, list[dict[str, Any]]]:
    ...
    shops = Shop.objects.filter(is_active=True).exclude(mvp_rate_card__isnull=True).exclude(mvp_rate_card={}).only("id", "mvp_rate_card")
    for shop in shops:
        rate_card = shop.mvp_rate_card or {}
        seen_keys: set[str] = set()
        for raw_row in rate_card.get("paper_rows") or []:
            key = str(raw_row.get("key") or "").strip()
            if not key or key in seen_keys or key not in PAPER_DEFINITION_BY_KEY or not raw_row.get("active"):
                continue
            seen_keys.add(key)
            rows_by_key[key].append(_enrich_paper_row(raw_row, definition=PAPER_DEFINITION_BY_KEY[key]))
    return rows_by_key
...
def build_partner_market_rate_payload(*, user) -> dict[str, Any]:
        ...
        results.append(
            {
                ...
                "suggested_markup_percent": str((profile.default_markup_rate or DEFAULT_MARKUP_RATE) * Decimal("100")),
                "explanation": (
    ...
    return {
        "role": "partner",
        "default_markup_rate": str(profile.default_markup_rate or DEFAULT_MARKUP_RATE),
        "results": results,
    }
