...
def calculate_booklet_pricing(
    *,
    ...
    total_good_sheets = int(cover_breakdown.get("imposition", {}).get("good_sheets", 0) or 0) + int(insert_breakdown.get("imposition", {}).get("good_sheets", 0) or 0)
    final_finishing_lines = []
    final_finishings_total = Decimal("0")
    for selection in finishing_selections or []:
        rule = selection.get("finishing_rate")
        if not rule:
            continue
        ...
        line = compute_finishing_line(
            rule,
            quantity=quantity,
            good_sheets=total_good_sheets,
            selected_side=selection.get("selected_side", "both"),
        )
        final_finishing_lines.append(line)
        final_finishings_total += Decimal(line.total)
    finishing_total = lamination_total + binding_total + final_finishings_total
    subtotal = cover_subtotal + insert_subtotal + binding_total + final_finishings_total
    ...
    if final_finishing_lines:
        explanations.append(f"Final finishings: {len(final_finishing_lines)} backend finishing rule(s) applied after booklet assembly.")
